...
export class NvidiaNimProvider implements AIProvider {
  ...
  constructor(options: NvidiaNimOptions) {
    this.apiKey = (options.apiKey ?? '').replace(/^["']|["']$/g, '');
    this.baseUrl = (options.baseUrl ?? 'https://integrate.api.nvidia.com/v1').replace(/^["']|["']$/g, '');
    this.model = (options.model ?? 'meta/llama-3.1-70b-instruct').replace(/^["']|["']$/g, '');
  ...
  async complete(prompt: string, options: AIOptions = {}): Promise<AIResult> {
    ...
    if (this.logger) {
      this.logger.debug({
        msg: 'NVIDIA NIM completion completed',
        model: this.model,
